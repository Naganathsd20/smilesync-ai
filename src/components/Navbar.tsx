  ...
  return (
    <nav className="fixed top-3 left-1/2 -translate-x-1/2 w-[calc(100%-1rem)] sm:w-[calc(100%-1.5rem)] max-w-7xl z-50 transition-all duration-300">
      ...
      <div className="bg-card/75 dark:bg-card/65 backdrop-blur-xl border border-white/10 dark:border-white/10 border-border/60 shadow-xl shadow-black/10 rounded-2xl md:rounded-full px-3.5 sm:px-5 py-2 transition-all duration-300">
        ...
        {mobileMenuOpen && (
          <div className="xl:hidden mt-3 pt-3 border-t border-border/50 grid grid-cols-2 sm:grid-cols-3 gap-1.5 pb-1 max-h-[70vh] overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
            {navItems.map((item) => {
              const isActive = pathname === item.href;
              const Icon = item.icon;
              return (
